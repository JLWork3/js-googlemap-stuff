...
Parse.$ = jQuery;
var autocomplete, map, geocoder, mc, router;
...
var EditPlanView = Parse.View.extend({
	...
	submit: function() {
		var that = this;
		var org = this.$("#inputOrg").val();
		var position = this.$("#inputTitle").val();
		var location = this.$("#inputLocation").val();
		var id;
		if(this.$('#inputID')){
			id = this.$('#inputID').val();
		}
		if ((!org) || (!position) || (!location)) {
			...
			getMapMarker(location, function(mapMarker) {
				...
				sp.save(null, {
					success: function(sp) {
						console.log("The profile is succeessfully saved!");
						$('#plan-submit-btn').button('reset');
						$("#modal").modal('hide');
						initializeMarkers();
						map.setZoom(10);
						map.setCenter(new google.maps.LatLng(mapMarker.get("geoLocation")._latitude, mapMarker.get("geoLocation")._longitude));
						$("#share-plan-btn-content").html("&nbsp;Update My Profile");
						$("#share-plan-btn").attr("onclick", "router.navigate('plan/" + sp.id + "',{trigger:true})");
					},
    ...
    FB.getLoginStatus(function(response) {
      if (response.status === 'connected') {
		...
		Parse.FacebookUtils.logIn("email", {
			success: function(user) {
				if (!user.existed()) {
					getFacebookId(function(id) {
						FB.api(id, {fields: 'email, first_name, last_name, picture.height(160), gender'}, function(response) {
          ...
          $('#login').button('reset');
          /*if (Parse.User.current() == user && loggedFB == true) {
...
var NavBarMenuView = Parse.View.extend({
	...
	render: function() {
		...
		// Toggle between the "Create new profile" and "Update my profile" content
			...
			query.find({
				success: function(results) {
				    ...
				    if (results.length == 1){
				    	...
				    	$("#share-plan-btn-content").html("&nbsp;Update My Profile");
						$("#share-plan-btn").attr("onclick", "router.navigate('plan/" + object.id + "',{trigger:true})");
				    }
				    else if (results.length == 0){
				    	$("#share-plan-btn-content").html("&nbsp;Create My Profile");
						$("#share-plan-btn").attr("onclick", "router.navigate('plan/new',{trigger:true})");
				    }
				    else{
				    	console.log("Error: User has less than 0 or more than 1 plans. Not allowed");
				    }
				},
				error: function(results) {
					console.log("Error: Querying plan failed.")
				}
			});
		}
		else{
			...
			(new Parse.Query("PlanLike")).include('user').containedIn("plan",plans.models).find().done(function(results){
				...
				if(plans.models.length){
					...
					if(title == "$PersonsPlans"){
						title = plans.models[0].get("parent").get("firstName") + " " + plans.models[0].get("parent").get("lastName") + "'s Profile";
...
function initializeMarkers() {
  ...
  var mcOptions = {gridSize: 1, maxZoom: 15};
...
function autocompleteInitialize() {
	...
	console.log("autocomplete successfully initialized" + autocomplete);
}
...
var UserPlansView = Parse.View.extend({
	render: function(user) {
		if(typeof user == "string"){
			...
			planCollectionView.render("My Profile",null);
		}else{
...
var AppRouter = Parse.Router.extend({
	...
	newPlan:function(){
		if (Parse.User.current()) {
			...
			query.find({
				...
				error: function(results) {
					console.log("Error: Querying plan failed. In newPlan()")
				}
			});
		} else {
			this.afterLogin = "plan/new";
	...
	logout:function(){
      ...
      if(loggedFB == false) {
        FB.getLoginStatus(function(response) {
          if (response.status === 'connected') {
            console.log("logging out of FB session");
